...
def evaluate_frac(data, position):
    def as_crit(span, x, y):
        smoothed = sm.nonparametric.lowess(y, x, frac=span, return_sorted=False)
        residuals = y - smoothed
        traceL = len(smoothed) * span  # Approximation for traceL
        sigma2 = np.sum(residuals ** 2) / (len(y) - 1)
        aicc = np.log(sigma2) + 1 + 2 * (2 * (traceL + 1)) / (len(y) - traceL - 2)

        return aicc

    def opt_span(x, y):
        result = minimize_scalar(lambda span: as_crit(span, x, y), bounds=(0.05, 0.95), method='bounded')
        return result.x

    position = np.array(position) / 1e6
    data = np.array(data)
    print("Get optimal window proportion...")
    best_frac = opt_span(position, data)
    print("Optimal window proportion:", best_frac)
...
def confidence_interval_plot(dict, data, position, chrome_set, save_path):
    this_dic = {}
...
def cal_function(x, y):
    ...
    k, b = curve_fit(fitting_function, x, y)[0]
    return abs(k)
...
class Statistic(object):
    ...
    def run(self):
        print(f"Running for function: {self.func_name}")
        ...
        for chrome_index in range(self.data.shape[0]):
            print("chrome:", self.chrome_set[chrome_index])
        ...
        plt.savefig(os.path.join(self.save_path,
                                 "{}-{}-{}-{}-{}.pdf".format(self.read_number, self.func_name, self.smooth_func,
                                                             self.smooth_window_size, self.threshold)),
                    format='pdf', bbox_inches='tight', dpi=fig.dpi, pad_inches=0.0)

        # np.save(os.path.join(self.save_path, "all_data_for_percentile_{}.npy".format(self.func_name)),
        #         all_data_for_percentile)
        # np.save(os.path.join(self.save_path, "all_data_for_plot_{}.npy".format(self.func_name)),
        #         np.array(all_data_for_plot, dtype=object))
        # np.save(os.path.join(self.save_path, "smooth_data_for_plot_{}.npy".format(self.func_name)),
        #         np.array(smooth_data_for_plot, dtype=object))
        # 保存为txt
