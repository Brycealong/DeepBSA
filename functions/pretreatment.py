...
class Pretreatment(object):
    ...
    def first_check(self, A_pool, a_pool):
        ...
        if np.all(p_values >= 0.05):
            return True
                ...
                (A_ratios[0] > 0.5 > A_ratios[-1]) or (A_ratios[0] < 0.5 < A_ratios[-1])):
            return True
    ...
    def second_check(self, A, a, freq, pos, is_process):
        ...
        :param freq: A的频率
        :param pos:  位置
        :param is_process: Continuity test
    ...
    def run(self, return_path):
        ...
        chrome_set = data[0].unique().tolist()
...
# pretreatment = Pretreatment(0, 1, 1, "../../Excel_Files/test.csv", "111", ".")
# pretreatment.run(0)
